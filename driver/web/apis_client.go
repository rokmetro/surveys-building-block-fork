...
func (h ClientAPIsHandler) createSurveyResponse(l *logs.Log, r *http.Request, claims *tokenauth.Claims) logs.HTTPResponse {
	...
	createdItem, err := h.app.Client.CreateSurveyResponse(model.SurveyResponse{UserID: claims.Subject, AppID: claims.AppID, OrgID: claims.OrgID, Survey: item}, claims.ExternalIDs, claims.Username, l)
	if err != nil {
...
func (h ClientAPIsHandler) getLeaderboardScores(l *logs.Log, r *http.Request, claims *tokenauth.Claims) logs.HTTPResponse {
	vars := mux.Vars(r)
	leaderboardID := vars["id"]
	if len(leaderboardID) <= 0 {
		return l.HTTPResponseErrorData(logutils.StatusMissing, logutils.TypePathParam, logutils.StringArgs("id"), nil, http.StatusBadRequest, false)
	...
	scores, err := h.app.Client.GetLeaderboardScores(leaderboardID, claims.OrgID, claims.AppID, &limit, &offset)
	if err != nil {
